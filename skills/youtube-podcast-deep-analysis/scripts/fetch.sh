...
CACHE_ROOT="${YTPDA_CACHE_DIR:-${XDG_CACHE_HOME:-$HOME/.cache}/youtube-transcripts-to-obsidian}"
...
if [[ ! -x "$PYTHON_ENV/bin/python" ]] || ! "$PYTHON_ENV/bin/python" -c 'import yt_dlp' >/dev/null 2>&1 || [[ ! -x "$DEFUDDLE_PATH" ]]; then
  if [[ "${YTPDA_ALLOW_INSTALL:-0}" != "1" ]]; then
    echo "Cached yt-dlp/Defuddle tools are unavailable. No installation was attempted." >&2
    echo "Set YTPDA_ALLOW_INSTALL=1 only after the user explicitly authorises installation." >&2
    exit 1
  fi
...
if [[ "${1:-}" == "--collection" ]]; then
  shift
  exec "$PYTHON_ENV/bin/python" "$SCRIPT_DIR/fetch_collection.py" "$@" --catalog "$SCRIPT_DIR/../references/ai-sources.json"
fi
exec "$PYTHON_ENV/bin/python" "$SCRIPT_DIR/fetch_transcripts.py" "$@"
